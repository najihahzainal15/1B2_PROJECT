<?php
	...
	$uEmail = $_POST["email"];
	$uPhone = $_POST["phoneNo"];
	$sProgramme = $_POST["programme"];
	$sYear = $_POST["year_of_study"];
	...
	$stmt = mysqli_prepare($link, "SELECT password FROM user WHERE userID = ?");
	mysqli_stmt_bind_param($stmt, "i", $uid2);
	...
	if (!password_verify($currpass, $hashedPassword)) {
	...
	$queryUser = "UPDATE user SET username = '$uName', email = '$uEmail', phone_No = '$uPhone', password = '$newHashedPass'
	WHERE userID = '$uid2'";
	$resultUser = mysqli_query($link,$queryUser);

	// 4. Update student table
	$queryStudent = "UPDATE student SET programme = '$sProgramme', year_of_study = '$sYear'
	WHERE userID = '$uid2'";
	$resultStudent = mysqli_query($link,$queryStudent);

	// 5. Redirect on success
	if ($resultUser && $resultStudent) {
		echo "<script type='text/javascript'>window.location='s_editProfile2.php'</script>";
	} else {
		die("Update failed.");
	}
?>
...
<body>
  ...
  <form action="s_editProfile2.php" method="POST" class="form"> 
		...
		<label>Phone Number</label><br>
			<input type="tel" name="phoneNo" placeholder="Enter your phone number" class="details2" value="<?php echo $uPhone; ?>"><br>
		
		<label>Programme</label><br>
			<select name = "programme" class="select1" value="<?php echo $sProgramme; ?>">
		...
		<label>Year of Study</label><br>
			<select name = "year" class="select2" value="<?php echo $sYear; ?>">
